...
const storage = new ChromeLocalStorageAdapter();
let state: AppState;
let statusMessage = "";
...
async function initialize(): Promise<void> {
  ...
  try {
    ...
    const firstOpenedAt = state.firstOpenedAt ?? new Date().toISOString();
    const locale = state.locale ?? resolveLocale(navigator.language);

    if (!state.firstOpenedAt || !state.locale) {
      await storage.save({ firstOpenedAt, locale });
    }
    ...
    renderLoadError(resolveLocale(navigator.language));
  }
}
...
function render(): void {
  ...
  const today = buildTodayViewModel(new Date(), locale);
  ...
  app.replaceChildren(
    createHeader(locale),
    createTodayBoard(locale, today.dateText, today.weekdayText),
    createPlannedItemCard(locale),
    createEditor(locale),
    createPremiumCard(locale, premium),
    createFooter(locale),
  );
}

function createHeader(locale: SupportedLocale): HTMLElement {
  ...
  const jaButton = createLanguageButton(locale, "ja", text(locale, "japanese"));
  const enButton = createLanguageButton(locale, "en", text(locale, "english"));
...
function createTodayBoard(locale: SupportedLocale, dateText: string, weekdayText: string): HTMLElement {
...
function createPlannedItemCard(locale: SupportedLocale): HTMLElement {
  const section = element("section", "planned-card");
  const heading = element("h2", "section-title", text(locale, "plannedItemLabel"));
  const plannedText = state.plannedItem.text || text(locale, "noPlannedItem");
  const value = element("p", state.plannedItem.text ? "planned-value" : "planned-value empty", plannedText);
  section.append(heading, value);
  return section;
}

function createEditor(locale: SupportedLocale): HTMLElement {
  ...
  input.type = "text";
  input.maxLength = 80;
  input.value = state.plannedItem.text;
...
function createFooter(locale: SupportedLocale): HTMLElement {
...
async function savePlannedItem(rawValue: string): Promise<void> {
  ...
  await storage.save({ plannedItem });
  state = {
    ...state,
    plannedItem,
  };
...
async function updateLocale(locale: SupportedLocale): Promise<void> {
