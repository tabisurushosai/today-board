...
- `src/storage/` は保存アダプタとシリアライズを担当します。
  ...
  - プラットフォーム固有コードはアプリ状態を直接読み書きせず、
    ...
    読み書きだけを実装します。読み込みは `read(keys)` で要求されたキーだけを
    返し、Chrome storage 全体取得のような特定 backend の挙動に依存しないで
    ください。
  ...
  - `serialization.ts` は現在の保存キー
    ...
    `premiumPurchasedAt`、`locale` を維持します。アプリが所有するキー一覧は
    `APP_STORAGE_KEY_LIST` に集約します。
...
- `StorageAdapter.read(keys)` は `APP_STORAGE_KEY_LIST` で渡されるアプリ所有キーに
